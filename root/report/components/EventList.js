...
const EventList = ({items, pager}: {items: $ReadOnlyArray<ReportEventT>, pager: PagerT}) => (
      ...
      <tbody>
        {items.map((item, index) => (
          <tr className={loopParity(index)} key={item.event.id}>
            <td>
              <EntityLink entity={item.event} showDisambiguation showEventDate={false} />
            </td>
            <td>{item.event.typeName ? lp_attributes(item.event.typeName, 'item.event_type') : null}</td>
            <td>
