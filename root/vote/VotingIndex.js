  ...
  return (
    <Layout fullWidth title={l('Voting suggestions')}>
      <div id="content">
        ...
        <p>
          {exp.l(
            `If you’d like to help ensuring the changes made to MusicBrainz
             data are correct, but do not know where to start, the following
             suggestions should be useful. These are pre-defined
             {edit_search|edit searches}; once you’re comfortable with the
             edit search you can refine them further or just create your own
             personal searches and bookmark them for your own use!`,
        ...
        <p>
          {exp.l(
            `While reviewing the work of your fellow editors, always keep the
             {coc|Code of Conduct} in mind. Almost all editors want to help,
             so your goal as a voter is to help them help better. This applies
             even more for beginners: always try to be helpful and patient
             with them, even if they are making mistakes, so that they’ll
             hopefully grow into better editors! That said, if you find
             an editor that seems to be vandalizing the data, you can always
        ...
        <p>
          {l(
            `Destructive edits (removals and merges) are often very hard or
             even impossible to revert. As such, an incorrect destructive edit
             that applies unnoticed can cause quite a big mess! Most are
        ...
        <ul>
          {/* eslint-disable eslint-plugin-js/max-len */}
          <VotingGuideRow
          ...
          <VotingGuideRow
            ...
            showSubscribedArtistsUrl
          />
          {/* eslint-enable eslint-plugin-js/max-len */}
        </ul>

        <h2>{l('Unreviewed and potentially problematic edits')}</h2>
        <p>
          {l(
            `Edits that nobody have seen can always benefit from a quick
             check: even if you’re not familiar with the music in question,
             you might be able to notice that something seems wrong. Don’t
             forget you don’t need to vote on every edit: it’s perfectly fine
             to just abstain if you feel something doesn’t seem wrong but it’s
             also not 100% obvious that it is right without further checks.`,
          )}
        </p>
        <ul>
          {/* eslint-disable eslint-plugin-js/max-len */}
        ...
        <p>
          {exp.l(
            `Beginner editors are the ones most likely to need a friendly eye
             on their edits. When reviewing these, look for cases where the
             editor is making edits that go against the MusicBrainz
             guidelines, or seem otherwise wrong, and leave notes guiding the
             editor so that they can do better in the future. Make sure to
             link to the appropriate guidelines, or to relevant examples of
             well-entered data, and above all remember to be nice. Chances are
             if these users are making a terrible mess, they’re not doing it
             on purpose but out of confusion: MusicBrainz can be daunting for
             newcomers! If at all possible, fix the errors (and let the editor
             know that you’ve done that and that they can check the edits you
             made to see how it should look like) rather than voting “No” on
             edits, since “No” votes can be quite discouraging, especially as
             a new editor.
        ...
        <p>
          {l(
            `The filters for edits you haven’t voted on yet below will only
             show open edits (since you can’t vote on closed edits anyway).
