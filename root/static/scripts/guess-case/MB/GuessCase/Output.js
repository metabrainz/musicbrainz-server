...
MB.GuessCase.Output = function (gc) {
    ...
    if ((!gc.mode.isSentenceCaps() || overrideCaps) &&
      ...
      if (w.match(/^\w\..*/) == null) {
        ...
        if (!overrideCaps &&
          ...
          let bef = gc.i.getPos(); let pos = bef-1;
  ...
  self.appendWordPreserveWhiteSpace = function (c) {
    if (c) {
      let ws = { before: gc.i.isPreviousWord(" "), after: gc.i.isNextWord(" ") };
      if (c.apply) {
        /*
