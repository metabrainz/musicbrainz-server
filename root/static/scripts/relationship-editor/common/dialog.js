...
import {compare, l, strings} from '../../common/i18n';
...
const RE = MB.relationshipEditor = MB.relationshipEditor || {};
    ...
    class Dialog {
        ...
        linkTypeName() {
            ...
            return linkTypeID ? linkPhrase.stripAttributes(linkTypeID, this.backward()) : "";
        }
        ...
        targetTypeOptions() {
            ...
            var options = _.map(targetTypes, function (type) {
                return { value: type, text: strings.entityName[type] };
            });
        ...
        sameEntityTypesLabel($parent, relationship, entity) {
            ...
            return l('Only relationships to {entity_type} entities.', {
                entity_type: strings.entityName[entityType].toLocaleLowerCase(),
            });
        }
