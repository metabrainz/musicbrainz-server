...
ko.bindingHandlers.relationshipEditorAutocomplete = (function () {
  ...
  function changeTarget(data) {
    ...
    let relationship = dialog.relationship();
    let entities = relationship.entities().slice(0);
    ...
    init: function (element, valueAccessor) {
      ...
      dialog.autocomplete = $(element).entitylookup({
        ...
        setEntity: function (type) {
          ...
          let possible = dialog.targetTypeOptions();
      ...
      let target = dialog.relationship().target(dialog.source);
...
ko.bindingHandlers.instrumentSelect = {
  ...
  init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
    let relationship = valueAccessor();
    let instruments = ko.observableArray([]);

    function addInstrument(instrument, linkAttribute) {
      let observable = ko.observable(instrument);
    ...
    let vm = {
      ...
      removeItem: function (item) {
        ...
        let $nextButton = $(element).find("button.remove-item:eq(" + index + ")");
    ...
    let childBindingContext = bindingContext.createChildContext(vm);
...
class Dialog {
  ...
  accept() {
    if (!this.hasErrors()) {
      ...
      for (let role in this.changeOtherRelationshipCredits) {
  ...
  clickEvent(data, event) {
    if (!event.isDefaultPrevented()) {
      let $menu = this.$dialog.find(".menu");
  ...
  changeDirection() {
    let relationship = this.relationship.peek();
  ...
  linkTypeName() {
    let linkType = this.relationship().getLinkType();
    if (!linkType) {
      return '';
    }
  ...
  linkTypeOptions(entityTypes) {
    ...
    if (this.source.entityType === "series") {
      let itemType = MB.seriesTypesByID[this.source.typeID()].item_entity_type;

      options = _.reject(options, function (opt) {
        let linkType = linkedEntities.link_type[opt.value];
  ...
  targetTypeOptions() {
    let sourceType = this.source.entityType;
    let targetTypes = _.without(MB.allowedRelations[sourceType], 'url');

    if (sourceType === "series") {
      let self = this;

      targetTypes = _.filter(targetTypes, function (targetType) {
        let key = [sourceType, targetType].sort().join("-");
    ...
    let options = _.map(targetTypes, function (type) {
  ...
  targetTypeChanged(newType) {
    ...
    let currentRelationship = this.relationship();
    let currentTarget = currentRelationship.target(this.source);

    let data = currentRelationship.editData();
    ...
    let entityTypes = [this.source.entityType, newType].sort().join("-");
    data.linkTypeID = defaultLinkType({ children: linkedEntities.link_type_tree[entityTypes] });
    data.attributes = [];

    let newRelationship = this.viewModel.getRelationship(data, this.source);
  ...
  linkTypeError() {
    ...
    if (!linkType) {
      ...
      let checker = URLCleanup.validationRules[linkType.gid];
  ...
  targetEntityError() {
    ...
    let target = relationship.target(this.source);
  ...
  dateError(date) {
    let valid = dates.isDateValid(date.year(), date.month(), date.day());
  ...
  datePeriodError() {
    ...
    let a = relationship.begin_date;
    let b = relationship.end_date;
...
Object.assign(Dialog.prototype, {
  ...
  setupUI: _.once(function () {
    let $dialog = $("#dialog").dialog(this.uiOptions);

    let widget = $dialog.data("ui-dialog");
...
function addRelationships(relationships, source, viewModel) {
  _.each(relationships, function (relationship) {
    ...
    if (relationship.getLinkType().orderable_direction) {
      let group = source.getRelationshipGroup(relationship, viewModel);
...
export class EditDialog extends Dialog {
  ...
  _accept() {
    let relationships = splitByCreditableAttributes(this.relationship());
    let relationship = relationships.shift();
...
export class BatchRelationshipDialog extends Dialog {
  ...
  _accept(callback) {
    ...
    let model = _.omit(this.relationship().editData(), "id", "entities");
...
export class BatchCreateWorksDialog extends BatchRelationshipDialog {
  ...
  accept() {
    ...
    let workLang = this.workLanguage();

    this.loading(true);

    let edits = _.map(this.sources, function (source) {
      let editData = MB.edit.fields.work({
        name: source.name,
        typeID: workType,
    ...
    this.createEdits(edits)
      .done((data) => {
        let works = _.map(data.edits, "entity");
...
function splitByCreditableAttributes(relationship) {
  ...
  let creditable = _.filter(attributes, isCreditable);
  ...
  let notCreditable = _.reject(attributes, isCreditable);

  function split(attribute) {
    let newRelationship = relationship.clone();
