...
$.widget('mb.entitylookup', $.ui.autocomplete, {
  ...
  _lookupSuccess: function (response, data) {
    ...
    const results = this.currentResults = _.filter(
      this.currentResults,
      function (item) {
        return !item.action;
      },
...
MB.Control.autocomplete_formatters = {
  ...
  'release': function (ul, item) {
    ...
    _(item.labels)
      ...
      .each(function (releaseLabels, name) {
        const catalogNumbers = _(releaseLabels).map(getCatalogNumber).compact().sort().value();
  ...
  'event': function (ul, item) {
    ...
    const entityRenderer = function (prefix, related_entities) {
      if (related_entities && related_entities.hits > 0) {
        const toRender = related_entities.results;
