  ...
  handleUrlChange(index: number, event: SyntheticEvent<HTMLInputElement>) {
    ...
    this.setLinkState(index, {url: url}, () => {
      if (!link.type) {
        ...
        if (type) {
          this.setLinkState(index, {type: typeInfo.link_type[type].id});
        }
      }
    });
  }
      ...
      <table id="external-links-editor" className="row-form">
        <tbody>
          {linksArray.map((link, index) => {
            var error;
            var linkType = link.type ? typeInfo.link_type[link.type] : {};
...
export class ExternalLink extends React.Component<LinkProps> {
  render() {
    ...
    var linkType = props.type ? typeInfo.link_type[props.type] : null;
    var typeDescription = '';
...
MB.createExternalLinksEditor = function (options: InitialOptionsT) {
  ...
  initialLinks.sort(function (a, b) {
    var typeA = a.type && typeInfo.link_type[a.type];
    var typeB = b.type && typeInfo.link_type[b.type];
  ...
  var typeOptions = (
    linkTypeOptions({children: typeInfo.link_type_tree[entityTypes]}, /^url-/.test(entityTypes))
