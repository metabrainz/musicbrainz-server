    ...
    while (true) {
      ...
      if (b) {
        ...
        if (m.name) {
          ...
          const expandedCredits = expandCredit(m.name, artists, isProbablyClassical);
...
function extractFeatCredits(str, artists, isProbablyClassical, allowEmptyName) {
  ...
  const m2 = extractNonBracketedFeatCredits(m1.name, artists, isProbablyClassical);
...
function expandCredit(fullName, artists, isProbablyClassical) {
   ...
   * or artist credit).
   */
  const bestFullMatch = bestArtistMatch(artists, fullName, isProbablyClassical);
...
export default function guessFeat(entity) {
  ...
  const match = extractFeatCredits(name, relatedArtists, isProbablyClassical, false);
