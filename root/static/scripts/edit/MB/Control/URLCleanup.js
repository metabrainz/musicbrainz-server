...
MB.Control.URLCleanup = function (sourceType, typeControl, urlControl, errorsObservable, showErrors) {
    ...
    validationRules[ MB.constants.LINK_TYPES.vgmdb.artist ] = function (url) {
        return url.match(/vgmdb\.net\//) != null;
    }
    ...
    validationRules[ MB.constants.LINK_TYPES.coverart.release ] = function (url) {
    ...
    var typeChanged = function(event) {
        ...
        if (!linkType) {
            self.errors.push(MB.text.SelectURLType);
        }
        else if (checker && !checker(url)) {
            self.errors.push(MB.text.InvalidURL);
        }
    };
