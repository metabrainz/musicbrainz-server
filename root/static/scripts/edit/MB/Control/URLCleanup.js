...
MB.constants.CLEANUPS = {
    wikipedia: {
        ...
        clean: function(url) {
            ...
            if ((m = url.match(/^(.*\.wikipedia\.org\/wiki\/)([^?#]+)(.*)$/)) != null)
                ºurl = m[1] + encodeURIComponent(decodeURIComponent(m[2])).replace(/%20/g, "_").replace(/%24/g, "$").replace(/%2C/g, ",").replace(/%2F/g, "/").replace(/%3A/g, ":").replace(/%3B/g, ";").replace(/%40/g, "@") + m[3];
            return url;
        }
    },
    discogs: {
    ...
    myspace: {
        ...
        clean: function(url) {
            return url.replace(/^(https?:\/\/)?(www\.)?myspace\.com/, "http://www.myspace.com");
        }
    },
    purevolume: {
