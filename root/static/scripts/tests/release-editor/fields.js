...
fieldTest((
  'release group types being preserved after editing the name'
), function (t, release) {
  t.plan(2);
  ...
  t.deepEqual(
    releaseGroup().secondaryTypeIDs(),
    [1, 3, 5],
    'secondary types are preserved',
  );
...
fieldTest((
  'mediums having their "loaded" observable set correctly'
), function (t, release) {
  t.plan(6);
  ...
  t.equal(
    mediums()[0].loaded(),
    true,
    'medium without id or tracks is considered loaded',
  );
  t.equal(
    mediums()[1].loaded(),
    true,
    'medium without id but with tracks is considered loaded',
  );
  t.equal(
    mediums()[2].loaded(),
    false,
    'medium with id but without tracks is considered not loaded',
  );
  t.equal(
    mediums()[3].loaded(),
    false,
    'medium with originalID but without tracks is considered not loaded',
  );
  t.equal(
    mediums()[4].loaded(),
    true,
    'medium with id and with tracks is considered loaded',
  );
  t.equal(
    mediums()[5].loaded(),
    true,
    'medium with originalID and with tracks is considered loaded',
  );
});

fieldTest((
  'loading a medium doesn’t overwrite its original edit data'
), function (t, release) {
  t.plan(11);
  ...
  medium.tracksLoaded({
    tracks: [{length: 12345, name: '~fooo~', position: 1}],
  });
...
fieldTest((
  'data tracks are appended with a correct position if there’s a pregap (MBS-8013)'
), function (t, release) {
  t.plan(1);
...
fieldTest((
  'tracks are set correctly when the cdtoc is changed'
), function (t, release) {
  t.plan(7);
...
fieldTest((
  ...
  tests.forEach(({input, output}) => {
    ...
    t.equal(
      medium.tracks()[0].formattedLength(),
      output,
      'length ' + input + ' is formatted as ' + output,
    );
  });
});
