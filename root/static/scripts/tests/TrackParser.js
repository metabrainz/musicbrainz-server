...
test('Tracknumbers', function () {
    ...
    $.each(tests, function (idx, test) {
        ...
        $.each (test.expected, function (idx, expected) {
            ...
            equal(r.title, expected.title, expected.title);
        });
    });
});
...
test('BugFixes', function () {
    ...
    $.each(tests, function (idx, test) {
        ...
        $.each (test.expected, function (idx, expected) {
            var r = result[idx];
            equal(r.position, idx+1, test.bug + ', ' + expected.title);
            equal(r.title, expected.title, test.bug + ', ' + expected.title);
            equal(r.duration, expected.duration, test.bug + ', ' + expected.title);
        });
    });
});
