...
function doArtistName(theForm, fid) {
	...
	if (newvalue != oldvalue) {
		...
		afUndo.addUndo(new UndoItem(theField, af_mode, oldvalue, newvalue));
	}
...
function doSortNameCopy(theForm, theArtistID, theSortID) {
	...
	if (newvalue != oldvalue) {
		...
		afUndo.addUndo(new UndoItem(theSortnameField, 'sortnamecopy', oldvalue, newvalue));
	}
	afCommons.resetSelection();
}
...
function doSortNameGuess(theForm, theArtistID, theSortID) {
	...
	if (newvalue != oldvalue) {
		theSortnameField.value = newvalue;
		afUndo.addUndo(new UndoItem(theSortnameField, 'sortname', oldvalue, newvalue));
	}
	afCommons.resetSelection();
}
...
function doSwapFields(theForm) {
	if (theForm.search && theForm.trackname && theForm.swapped) {
		...
		afUndo.addUndo(new UndoItemList(
						new UndoItem(theForm.trackname, 'swap', theForm.trackname.value, theForm.search.value),
						new UndoItem(theForm.search, 'swap', theForm.search.value, theForm.trackname.value),
						new UndoItem(theForm.swapped, 'swap', theForm.swapped.value, newSwapped)
					   ));
...
function doUseCurrent(theForm) {
	if (theForm != null) {
		if (theForm.search != null && theForm.orig_artname != null &&
			...
			afUndo.addUndo(new UndoItemList(
							new UndoItem(theForm.trackname, 'usecurrent', theForm.trackname.value, theForm.orig_track.value),
							new UndoItem(theForm.search, 'usecurrent', theForm.search.value, theForm.orig_artname.value)
						   ));
...
function doUseSplit(theForm) {
	if (theForm != null) {
		if (theForm.split_artname != null &&
			...
			afUndo.addUndo(new UndoItemList(
							new UndoItem(theForm.trackname, 'usesplit', theForm.trackname.value, theForm.split_track.value),
							new UndoItem(theForm.search, 'usesplit', theForm.search.value, theForm.split_artname.value)
						   ));
...
function doArtistAndTrackName(theForm) {
	if (theForm != null) {
		...
		var tnUndo = new UndoItem(theForm.trackname, 'guessboth', tnOldValue, tnValueFixed);
		var snUndo = new UndoItem(theForm.search, 'guessboth', snOldValue, snValueFixed);
...
function af_artistNameFix(v) {
	return artistNameFix(v);
}
...
// -- Atomic operation, which take the value and applies
//    the current selected autofix operation
function af_albumNameFix(v) {
	return albumNameFix(v, af_mode);
}
...
// -- Atomic operation, which take the value and applies
//    the current selected autofix operation
function af_trackNameFix(v) {
	return trackNameFix(v, af_mode);
}
