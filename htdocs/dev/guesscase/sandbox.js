// ----------------------------------------------------------------------------
// sb_errorHandler()
function sb_errorHandler(msg, url, lno) {
	bb = "";
	try {
		bb = document.body.innerHTML;
		bbarr = bb.split(/\n/g);
		bb = 	"\n#"+(lno-1)+ ": "+bbarr[lno-1]+"\n"+
				"#"+(lno)  + ": "+bbarr[lno]+"\n"+
				"#"+(lno+1)+ ": "+bbarr[lno+1];
	} catch (e) { bb = e; }
	alert(msg+", "+url+", "+lno+", "+bb)
	return true;
}
window.onError = sb_errorHandler;

// ----------------------------------------------------------------------------
// sb_testSplit()
function sb_testSplit() {
	resetMessages();
	addMessage("sb_testSplit() ... ");
	var val = document.getElementById("splittesttext").value;
	var result = splitWordsAndPunctuation(val);
	sb_logMessages();
}

// ----------------------------------------------------------------------------
// sb_logMessages()
function sb_logMessages() {
	document.getElementById("messages").innerHTML = getMessages();
}

// ----------------------------------------------------------------------------
// sb_writeButton()
// -- write a guess case button to the document.
function sb_writeButton(theType, theID, theID2) {
	var theTitle = '';
	var theFunction = '';
...
WebFXTabPane.prototype.addTabPage = function (oElement) {
	...
	try {
		var pane = oElement.tabPage;
		if (pane == this) return oElement.tabPage; 
	} catch (e) { /* catch ex */ }
	var n = this.pages.length;
	var tp = this.pages[n] = new WebFXTabPage(oElement, this, n);
	tp.tabPane = this;
	this.tabRow.appendChild(tp.tab); // move the tab out of the box	
...
function disposeAllTabs() {
	...
	for (var i = 0; i < l; i++) {
		...
		if (tabPaneRe.test(cn) && el.tabPane) // tab pane
