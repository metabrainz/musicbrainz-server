		...
		if (f.exists()) {
						...
						if (srcDir != null && buildDir != null) {
							if (key.equals("lib")) {
								...
								if (val.indexOf("*") == -1) {
									...
									System.out.println("Slurping... "+includeDir.getAbsolutePath());
			...
			if (deployDir != null && buildDir != null) {
				...
				for (Iterator it = files.iterator(); it.hasNext(); ) {
					System.out.print(((File)it.next()).getName()+(it.hasNext()?",":""));
				}
				System.out.println();
				...
				try {
					...
					for (Iterator it = files.iterator(); it.hasNext(); ) {
						in  = new BufferedReader(new FileReader((File)it.next()));
				...
				PrintStream psout = new PrintStream(new FileOutputStream(fpacked));
				Main.setOut(psout);
				Main.exec(new String[] {"-c", fin.getAbsolutePath()});
				psout.close();
				...
				/*
				Hashtable<Object, Object> g = new Hashtable<Object, Object>();
				g.put("error","");
				g.put("warning","");
				g.put("info","");
				g.put("debug","");
				...
				System.out.println();
				System.out.println("Replacing functions...");

				Hashtable<String, String> x = new Hashtable<String, String>();
				Pattern p = Pattern.compile("this\\.([a-z0-9]+)\\s*=\\s*function\\(", Pattern.CASE_INSENSITIVE);
				Matcher m = p.matcher(s);
				int i=0;
				while (m.find()) {
					String oname = m.group(1);
					if (g.get(oname.toLowerCase()) == null && x.get(oname) == null
					    && oname.length()>4) {
						String nname = "__"+(i++);
						System.out.println("* "+oname+" "+nname);
						x.put(oname, nname);
						s = s.replaceAll("\\."+oname+"\\(", "."+nname+"(");
						s = s.replaceAll("\\."+oname+" ", "."+nname+" ");
					}
				}

				p = Pattern.compile("this\\.([^_]([A-Z0-9_]+))");
				m = p.matcher(s);
				x = new Hashtable<String, String>();
				i=0;
				System.out.println();
				System.out.println("Replacing constants...");
				while (m.find()) {
					String oname = m.group(1);
					if (g.get(oname.toLowerCase()) == null && x.get(oname) == null
					    && oname.length()>4) {
