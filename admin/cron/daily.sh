...
carton exec -- ./admin/replication/BundleReplicationPackets $FTP_DATA_DIR/replication --period daily
if date +%w | grep -q [6]
then
    echo `date`" : + weekly"
    carton exec -- ./admin/replication/BundleReplicationPackets $FTP_DATA_DIR/replication --period weekly
fi
