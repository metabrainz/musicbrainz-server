...
if ! script/database_exists $DATABASE; then
    if ! compgen -G "$DIRECTORY/mbdump*.tar.bz2" > /dev/null; then
        RM_DUMPS=true
        curl -sSL -o $DIRECTORY/LATEST http://ftp.musicbrainz.org/pub/musicbrainz/data/fullexport/LATEST
        LATEST="$(cat $DIRECTORY/LATEST)"
        echo "Downloading dump: mbdump-derived.tar.bz2"
        curl -L -o $DIRECTORY/mbdump-derived.tar.bz2 --retry 15 -C - http://ftp.musicbrainz.org/pub/musicbrainz/data/fullexport/$LATEST/mbdump-derived.tar.bz2
        echo "Downloading dump: mbdump.tar.bz2"
        curl -L -o $DIRECTORY/mbdump.tar.bz2 --retry 15 -C - http://ftp.musicbrainz.org/pub/musicbrainz/data/fullexport/$LATEST/mbdump.tar.bz2
    fi
