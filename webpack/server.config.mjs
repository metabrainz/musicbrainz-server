...
export default {
  ...
  externals: [
    ...
    /*
     * Externalize DBDefs files so server.js loads them from disk at runtime
     * instead of baking values in at compile time. This allows server.js
     * to be compiled once at image build time and pick up runtime DBDefs
     * generated by dbdefs_to_js.pl at container start — eliminating the
     * need for webpack at container startup and reducing startup time.
     */
    function ({request}, callback) {
      if (/DBDefs(?:-client)?\.mjs$/.test(request)) {
