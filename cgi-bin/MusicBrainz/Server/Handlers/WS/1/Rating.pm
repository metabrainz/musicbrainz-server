...
sub handler
{
	...
	# POST http://server/ws/1/rating/?name=<user_name>&entity=<entity>&id=<id>&ratings=<ratings>
...
sub handler_post
{
	...
	my $status = eval 
    {
		...
		{
			my $status = serve_from_db_post($r, $user, $entity, $id, $ratings);
...
{
	my ($r, $user, $entity, $id, $ratings) = @_;

	my $printer = sub {
		print_xml_post($user, $entity, $id, $ratings);
	};
...
sub print_xml_post
{
    ...
    my $rating = MusicBrainz::Server::Rating->new($mb->{DBH});
	print STDERR "Hi mom!\n";
    $rating->Update($ratings, $us->GetId, $entity, $obj->GetId);
