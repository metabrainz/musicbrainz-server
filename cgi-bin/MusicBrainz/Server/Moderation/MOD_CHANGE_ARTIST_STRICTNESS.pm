...
package MusicBrainz::Server::Moderation::MOD_CHANGE_ARTIST_STRICTNESS;

use ModDefs qw( :modstatus MODBOT_MODERATOR );
use base 'Moderation';

sub Name { "Change Edit Strictness Level" }
...
sub PreInsert
{
	...
	$self->SetPrev($artist->GetStrict);
	$self->SetNew($action);
	$self->SetTable("artist");
	$self->SetColumn("strict");
...
sub CheckPrerequisites
{
	...
	if ($artist->GetStrict == $self->GetNew)
	{
		$self->InsertNote(MODBOT_MODERATOR, "This artist is already set to level " . Artist::GetStrictName($artist->GetStrict));
...
1;
# eof MOD_CHANGE_ARTIST_STRICTNESS.pm
