...
sub GetAlbum	{ $_[0]{_album} ||= $_[0]->_GetAlbum }
...
# Given an album (object or ID), get a list of AlbumCDTOCs, including their
...
# Given a discid / freedbid, get a list of AlbumCDTOCs, including their CDTOCs
...
sub GetAlbumIDsFromDiscID { my $self = shift; $self->_GetAlbumIDsFromHash("discid", @_) }
sub GetAlbumIDsFromFreeDBID { my $self = shift; $self->_GetAlbumIDsFromHash("freedbid", @_) }

sub _GetAlbumIDsFromHash
{
	my $self = shift;
	my $type = shift;
	$self = $self->new(shift) if not ref $self;
...
sub GenerateAlbumFromDiscid
{
	...
	my $albumids = $self->GetAlbumIDsFromDiscID($discid);
