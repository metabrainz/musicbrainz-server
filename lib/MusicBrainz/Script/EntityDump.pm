...
sub releases {
    ...
    core_entity($c, 'release', $ids, sub {
        ...
        my $cover_art_ids = pluck('id', $cover_art_rows);
        handle_rows($c, 'cover_art_archive.cover_art_type', 'id', $cover_art_ids);
        handle_rows($c, 'cover_art_archive.release_group_cover_art', 'release', $ids);
    });
}
