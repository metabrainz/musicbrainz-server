...
sub authenticate {
    ...
    if (defined $token_instance) {
        my $client_id = $token_instance->metabrainz_client_id;
        # Only tokens for the actual MusicBrainz client should be accepted
        # for website logins. While there's no known way to get an arbitrary
        # access token here, it's verified just to be safe.
        return unless (
            defined $client_id &&
            $client_id eq DBDefs->METABRAINZ_OAUTH_CLIENT_ID
        );
