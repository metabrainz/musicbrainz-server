...
sub works : Chained('load')
{
    ...
    my $works = $self->_load_paged($c, sub {
        $c->model('Work')->find_by_artist($c->stash->{artist}->id, shift, shift, filter => \%filter);
    });
    ...
    my %props = (
        ajaxFilterFormUrl => '' . $c->uri_for_action('/ajax/filter_artist_works_form', { artist_id => $artist->id }),
