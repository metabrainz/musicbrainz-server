...
sub oauth2_redirect : Private {
    ...
    my $code_verifier = generate_token();
    my $code_challenge = encode_base64url(sha256($code_verifier), '');

    my $state_id = $self->_save_oauth2_redirect_state(
        $c,
        $local_path,
        $code_verifier,
    );
    ...
    $uri->query_param_append('code_challenge', $code_challenge);
    $uri->query_param_append('code_challenge_method', 'S256');
    $uri->query_param_append('login_hint', $login_hint)
        if defined $login_hint;
    $c->res->redirect($uri);
}

sub oauth2_callback : Chained('base') PathPart('oauth2/callback') Args(0) RequireSSL {
    my ($self, $c) = @_;
    ...
    my $error = $query_params->{error};
    if (non_empty($error)) {
        my $state_id = $query_params->{state};
        if (non_empty($state_id)) {
            $c->model('MB')->context->store->delete(
                "oauth2_redirect_state:$state_id",
            );
        }
        $c->flash->{message} = $error eq 'access_denied'
            ? l('Login cancelled.')
    ...
    unless (DBDefs->DB_READ_ONLY || DBDefs->DISABLE_LAST_LOGIN_UPDATE) {
        $c->model('Editor')->update_last_login_date($user->id);
    }
