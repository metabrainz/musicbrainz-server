...
sub secondary_counts {
    ...
    if ($viewing_own_profile || $editor->preferences->public_tags) {
        ...
        my @tag_tables = entities_with('tags', take => sub { return (shift) . "_tag_raw" });
        my $tag_inner_query = join(
            ' UNION ALL ',
            map { "SELECT is_upvote FROM $_ WHERE editor = ?" } @tag_tables
        );
    
        my $query = "SELECT x.is_upvote, count(*)
                     FROM ($tag_inner_query) x
                     GROUP BY x.is_upvote";

        my $rows = $self->sql->select_list_of_lists($query, ($editor_id) x scalar @tag_tables);
    ...
    if ($viewing_own_profile || $editor->preferences->public_ratings) {
        my @rating_tables = entities_with('ratings', take => sub { return (shift) . "_rating_raw" });
        ...
        $result{'rating_count'} = $self->sql->select_single_value($query, ($editor_id) x scalar @rating_tables);
    }
