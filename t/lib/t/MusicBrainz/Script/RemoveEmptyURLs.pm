...
test 'The RemoveEmpty script deletes the expected URLs' => sub {
    ...
    my $sql = $c->sql;
    is(
        $sql->select_single_value('SELECT COUNT(*) FROM url'),
        5,
    ...
    ok(
        !exception { $script->run('url') },
        'The removal script ran successfully',
    );
    ...
    is(
        $sql->select_single_value('SELECT 1 FROM url WHERE id = 2'),
        1,
        'Recently updated unused URL has not been deleted',
    );

    is(
        $sql->select_single_value('SELECT 1 FROM url WHERE id = 5'),
        1,
