...
test 'Release group type validation' => sub {
    ...
    is($mech->status, HTTP_BAD_REQUEST, 'A Bad Request error is returned');
    ...
    is($mech->status, HTTP_OK, 'No error is returned');
...
test 'Tag auth validation' => sub {
    ...
    isnt(
        $mech->status,
        HTTP_UNAUTHORIZED,
        'An Unauthorized error is not returned',
    );
    ...
    is($mech->status, HTTP_UNAUTHORIZED, 'An Unauthorized error is returned');
