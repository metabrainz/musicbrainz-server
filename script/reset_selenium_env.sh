...
SQL
    ...
    echo "$CANCEL_QUERY" | ./admin/psql SELENIUM -- --single-transaction -v ON_ERROR_STOP=1 2>&1
}
...
if [[ $SIR_DIR ]]; then
    ...
    ./script/purge_sir_queues.sh /sir-test 2>&1

    echo `date` : Purging Solr cores
    ./script/purge_solr_cores.sh 2>&1
else
    terminate_pg_backends
fi
